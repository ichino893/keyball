...
// 1. 同時押しするキーの組み（定義）
const uint16_t PROGMEM my_esc[]    = {KC_Q, KC_E, COMBO_END};     // Q + E (ESC)
const uint16_t PROGMEM my_btn1[]   = {KC_J, KC_K, COMBO_END};     // J + K (左クリック)
const uint16_t PROGMEM my_btn2[]   = {KC_K, KC_L, COMBO_END};     // K + L (右クリック)
...
// const uint16_t PROGMEM my_up[]     = {KC_K, KC_I, COMBO_END};
// const uint16_t PROGMEM my_right[]  = {KC_K, KC_L, COMBO_END};
// const uint16_t PROGMEM my_down[]   = {KC_K, KC_COMM, COMBO_END};
...
combo_t key_combos[] = {
    ...
    COMBO(my_btn1, KC_BTN1),     // J + K          -> マウス左クリック
    COMBO(my_btn2, KC_BTN2),     // K + L          -> マウス右クリック
