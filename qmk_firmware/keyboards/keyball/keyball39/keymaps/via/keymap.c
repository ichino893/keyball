...
// 1. 同時押しするキーの組み（定義）
const uint16_t PROGMEM my_esc[]    = {KC_Q, KC_E, COMBO_END};     // Q + E (※ESCはQEのまま残しています)
const uint16_t PROGMEM my_btn1[]   = {KC_J, KC_I, COMBO_END};     // J + I (元に戻しました)
const uint16_t PROGMEM my_btn2[]   = {KC_I, KC_O, COMBO_END};     // I + O (元に戻しました)
const uint16_t PROGMEM my_bspc[]   = {KC_O, KC_P, COMBO_END};     // O + P
const uint16_t PROGMEM my_p_mins[] = {KC_P, KC_MINS, COMBO_END}; // P + (Pの下)

const uint16_t PROGMEM my_left[]   = {KC_J, KC_K, COMBO_END};     // J + K (元に戻しました)
const uint16_t PROGMEM my_up[]     = {KC_K, KC_I, COMBO_END};     // K + I (元に戻しました)
const uint16_t PROGMEM my_right[]  = {KC_K, KC_L, COMBO_END};     // K + L (元に戻しました)
const uint16_t PROGMEM my_down[]   = {KC_K, KC_COMM, COMBO_END};  // K + , (元に戻しました)
